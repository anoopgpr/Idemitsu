...
var SERVER_WalletCheck = "https://taghash.co/grati/idemitsu/walletcheck.php";
...
$(document).ready(function(event){
	...
	document.addEventListener("backbutton", function(e)
	{
		//    if($.mobile.activePage.is('#form-btn-pg'))
		   ...
		   if($.mobile.activePage.is('#dealerhomepage') || $.mobile.activePage.is('#homepage'))
		   {
			   e.preventDefault();
			   var x;
...
function checkapp_Version()
{
     ...
     $.getJSON(
            'https://taghash.co/mloyal_appversion/getapp_version.php?appname=idemitsu',
			{
		    ...
		    function (json) {
				 ...
				 {
					show_updatePopup();
					//$.mobile.changePage( "#SelCatLogin", { transition: "none"} );
...
function checkapp_Version_homepage()
{
...
function authenticate_new()
{
  ...
  document.getElementById("usermobileshow").innerHTML= uname;
...
//    var dataa = localStorage.getItem('usernameidemitsu');
...
function showScan_new_check_main()
{
  ...
  //  $(".vehical-img-col-01, .my-2-wheeler").on("click", function () {
 //$("#scannerPopup").addClass("idem-active");
  //   });
...
function showScan_new_check_main_old()
{
	$.mobile.changePage( "#scanPage_options", { transition: "none"} );
	fetchcurrent_Location_scan();
}
	     ...
	     cordova.plugins.barcodeScanner.scan(
			   function (result) {
					...
					else
				    {
                        ...
                        // let trimTheBarCode = bcode1.substring(0,10);
...
function submit_scanResult_frompage()
{
	...
	{
		submit_scanResult_text(cpncode);
	}
}
...
function onFail(message)
{
	...
	toast('Please give Camera Permission for Pan Card Photo & QR Code Scanning');
}
...
$(document).on("pageshow", "#homepage", function(){
	...
	//setUserbalance_aboutus();
	update_balancePts();
	$('.submitpaytmwallet_1').css("pointer-events", "");
...
function submitPaytm()
{
	...
	else
	{
		 ...
		 $('.submitpaytmwallet_1').css("pointer-events", "none");
	}

}

function burnpoints_byqr_old(camount,userid)
{

	var refbillno =  'idem'+Math.floor((Math.random() * 100000000) + 1)+''+Math.floor(Date.now() / 1000);
...
function sendGratification_old(amount,userid,refbillno)
{
	    $.ajax({
				...
				dataType: 'json',
				headers: {
					"uid": "72e0ced034a7d6c00b364dfd8dcbc693",
					"pwd": "6c7a4b05c0f84192084ec0a1f3ca03d921f981a61bd182e52b922b102e5eaeff",
 ...
 if(user.store_code_idemitsu=='undefined' || user.store_code_idemitsu=='null' || user.store_code_idemitsu==null)
	...
	if(user.state.trim()!='')
	{
		if(user.state.match(numbers))
		{
			...
			$("#workshopState").val(user.state);
			fetchCitylist_1_new(user.state,'workshopPincode','workshopCity','workshopCity_other');
		}
		else
		{
			...
			fetchCitylist_1_new(sval,'workshopPincode','workshopCity','workshopCity_other');
		 }
	}
...
function authenticateDealer()
{
 ...
 var dist_code_lastdigits = '';
 
 if(user.distown_code!='undefined' && user.distown_code!='null' && user.distown_code!=null && user.distown_code!='')
 {
	dist_code_lastdigits = (user.distown_code).substring(6);
 }
 else
 {
	dist_code_lastdigits = user.distown_code;
 }


 console.log('dist_code_lastdigits:'+dist_code_lastdigits);
			...
			$('#distributorCode').val(dist_code_lastdigits);
			//$('#retailerCity').val(user.city);
			//$('#retailerState').val(user.state);
	...
	if(user.state.trim()!='')
	{
		...
		{
			$("#distributorState").val(user.state);
			fetchCitylist_1_new(user.state,'distributorPincode','distributorCity','distributorCity_other');
		}
		else
		{
			...
			fetchCitylist_1_new(sval,'distributorPincode','distributorCity','distributorCity_other');
		 }
	}
...
function authenticateRetailer()
{
 ...
 if(user.store_code_idemitsu=='undefined' || user.store_code_idemitsu=='null' || user.store_code_idemitsu==null)
	...
	if(user.state.trim()!='')
	{
		...
		{
			$("#retailerState").val(user.state);
			fetchCitylist_1_new(user.state,'retailerPincode','retailerCity','retailerCity_other');
		}
		else
		{
...
function authenticateMechanic()
{
 ...
 if(user.store_code_idemitsu=='undefined' || user.store_code_idemitsu=='null' || user.store_code_idemitsu==null)
	...
	if(user.docPhoto != '')
	 {
		...
		{
			//document.getElementById("shop_photo").src = versionurl+'uploads/'+user.panphoto;
			document.getElementById("kycdoc_photo3_machanic").src = user.docPhoto;
			localStorage.setItem('docimg',user.docPhoto);
		}
	 }
	...
	if(user.state.trim()!='')
	{
		...
		{
			$("#machanicState").val(user.state);
			fetchCitylist_1_new(user.state,'machanicPincode','machanicCity','machanicCity_other');
		}
		else
		{
			...
			fetchCitylist_1_new(sval,'machanicPincode','machanicCity','machanicCity_other');
		 }
	}
...
function checkdistributor(str)
{
	...
	$.ajax({
		...
		timeout: 50000,
		dataType: 'json',
		data: {'apiuid': 'MLOYALAPI', 'apipswd': 'Ml0yalAP!2o14','distributorcode': '235009'+data1},
		complete: function(xhr, textStatus) {
		},
		success: function(data, textStatus, xhr) {
			// console.log(data);
			...
			{
				if(data.data[0].MobileNo == user.mobile)
				{
					// do nothing
				}
				else
				{
					toast('Code is incorrect/Account already available');
					$('#'+str).val('');
				}
			}
		}
...
user.pan_no = checkforundefined(data.PAN_No);
 ...
 if(user.store_code_idemitsu=='undefined' || user.store_code_idemitsu=='null' || user.store_code_idemitsu==null)
	...
	if(user.docPhoto != '')
	 {
		...
		{
			//document.getElementById("shop_photo").src = versionurl+'uploads/'+user.panphoto;
			document.getElementById("kycdoc_photo3_consumer").src = user.docPhoto;
			localStorage.setItem('docimg',user.docPhoto);
		}
	 }
	...
	if(user.state.trim()!='')
	{
		...
		{
			$("#consumerState").val(user.state);
			fetchCitylist_1_new(user.state,'consumerPincode','consumerCity','consumerCity_other');
		}
		else
		{
...
function updateDistributor()
{
		...
		var mobile = user.mobile;
		var d_code = $('#distributorCode').val();
		var distributorCode = '235009'+d_code;
        var distributorCompanyName = $('#distributorCompanyName').val();
        var distributorOwnerName = $('#distributorOwnerName').val();
		...
		var distributorCity2 = $('#distributorCity_other').val();
		

		var docimg = localStorage.getItem('docimg');

		
	if(d_code.trim()=='' || distributorCompanyName.trim()=='' || distributorOwnerName.trim()=='' || distributorPincode.trim()=='' || distributorGstNo.trim()=='' || distributorPanno.trim()=='' || emailUpdate.trim()=='')
	{
		toast('Please enter mandatory fields.');
	}
	...
	else if(distributorCity2=='' || distributorCity2=='undefined' || distributorCity2=='null' || distributorCity2==null)
    {
	     toast('Please check city');
...
function updateWorkshop()
{
		...
		var workshopCity2 = $('#workshopCity_other').val();
	...
	if(workshoppincode.trim()=='' || workshopCompanyName.trim()=='' || workshopOwnerName.trim()=='' || workshopPanno.trim()=='')
	{
		toast('Please enter mandatory fields.');
	}
	...
	else if(workshopCity2=='' || workshopCity2=='undefined' || workshopCity2=='null' || workshopCity2==null)
    {
	     toast('Please check city');
		 $('#workshopCity').focus();
...
function updateRetailer()
{
		...
		var retailerCity2 = $('#retailerCity_other').val();
	...
	if(retailerPincode.trim()=='' || retailerCompanyName.trim()=='' || retailerOwnerName.trim()=='' || retailerPanno.trim()=='')
	{
		toast('Please enter mandatory fields.');
	}
	...
	else if(retailerCity2=='' || retailerCity2=='undefined' || retailerCity2=='null' || retailerCity2==null)
    {
	     toast('Please check city');
		 $('#retailerCity').focus();
...
function updateMechanic()
{
		...
		var machanicCity2 = $('#machanicCity_other').val();

		var machanicLocation = $('#machanicLocation').val();
		var machanicPanNo = $('#machanicPanNo').val();
...
var docimg = localStorage.getItem('docimg');
	...
	else if(machanicCity2=='' || machanicCity2=='undefined' || machanicCity2=='null' || machanicCity2==null)
    {
	     toast('Please check city');
		 $('#machanicCity').focus();
...
function updateConsumer()
{
		...
		var consumerCity2 = $('#consumerCity_other').val();
		...
		var consumerPanNo = $('#consumerPanNo').val();
...
var docimg = localStorage.getItem('docimg');
	...
	else if(consumerCity2=='' || consumerCity2=='undefined' || consumerCity2=='null' || consumerCity2==null)
    {
	     toast('Please check city');
		 $('#consumerCity').focus();
...
function submit_scanResult(cpncodeOld)
{
    var cpncode = cpncodeOld.substring(0, 10);
    alert("::::::::::::::>"+cpncode);
		   ...
		   //called when complete
			...
			success: function(data, textStatus, xhr) {
				 ...
				 if(data.success=='1')
				 {
					  ...
					  {
						  if(data.data.GET_PROMOTIONAL_COUPON_VALIDATION_INFOResult.message=='Unknown error found')
						  {
...
function submit_scanResult_text(cpncode)
{
	 ...
	 $.ajax({
		   ...
		   //called when complete
			...
			success: function(data, textStatus, xhr) {
				 ...
				 {
					 toast_p('No success response found..');
					 $('#scanpage_code').val("");
					 $.mobile.changePage( "#scanPage", {transition: "none"} );
				 }
...
function burnCoupon_text(cpncode)
{
    ...
    success: function(data, textStatus, xhr) {
		 ...
		 if(data.success=='1')
		 {
			  if(data.data.GET_PROMOTIONAL_COUPON_VALIDATION_INFOResult.Success === true)
			  {
				   ...
				   //console.log('cpnvalue:'+cpnvalue);
				   //sendGratification(cpnvalue);
				   earn_points_final_text(cpnvalue,cpncode);
			  }
			  else
			  {
				    toast(data.data.GET_PROMOTIONAL_COUPON_VALIDATION_INFOResult.message);
...
function earn_points_final_text(cpnvalue,cpncode)
{
	  //var refbillno = 'idemitsu_'+Math.floor((Math.random() * 1000000000) + 1);
	  var lat = checkforundefined($('#scanpage_latitude').val());
	  var longt = checkforundefined($('#scanpage_longitude').val());
...
function capturePhoto_kycdoc3_consumer()
{
	navigator.camera.getPicture(uploadPhoto_kycdoc3_consumer, onFail, { quality: 40,destinationType: navigator.camera.DestinationType.FILE_URI,targetWidth:512,targetWidth:512,correctOrientation:true});
}
function capturePhoto_kycdoc3_machanic()
{
	navigator.camera.getPicture(uploadPhoto_kycdoc3_machanic, onFail, { quality: 40,destinationType: navigator.camera.DestinationType.FILE_URI,targetWidth:512,targetWidth:512,correctOrientation:true});
}
...
function uploadPhoto_kycdoc3_machanic(imageURI) 
			{
			
							
							var b_Image = document.getElementById('kycdoc_photo3_machanic');
							b_Image.style.display = 'block';
							b_Image.src = imageURI;
							...
							//var cattype = localStorage.getItem('cattype');
							statusDom=document.getElementById("kycdoc_photo_status3_machanic");
							statusDomdiv=document.getElementById("kycdoc_photo_status_div3_machanic");
							...
							ft.upload(imageURI, encodeURI("https://taghash.co/idemitsu/upload_photo_new1.php"), win, failPhoto_kycdoc3_machanic, options, true);
			}
function uploadPhoto_kycdoc3_consumer(imageURI) 
			{
			
							
							var b_Image = document.getElementById('kycdoc_photo3_consumer');
							b_Image.style.display = 'block';
							b_Image.src = imageURI;
							...
							//var cattype = localStorage.getItem('cattype');
							statusDom=document.getElementById("kycdoc_photo_status3_consumer");
							statusDomdiv=document.getElementById("kycdoc_photo_status_div3_consumer");
							...
							ft.upload(imageURI, encodeURI("https://taghash.co/idemitsu/upload_photo_new1.php"), win, failPhoto_kycdoc3_consumer, options, true);
			}
...
function failPhoto_kycdoc3_consumer(error)
{
			toast('Photo upload failed. Please try after sometime: ' + error.code);
			localStorage.removeItem('docimg');
			doc_img = '';
			document.getElementById('kycdoc_photo3_consumer').src = "assets/images/camera_icon.png";
}
function failPhoto_kycdoc3_machanic(error)
{
			toast('Photo upload failed. Please try after sometime: ' + error.code);
			localStorage.removeItem('docimg');
			doc_img = '';
			document.getElementById('kycdoc_photo3_machanic').src = "assets/images/camera_icon.png";
}
...
function fetchCitylist_1_new(stateid,pincodeid,cityid,cityidother)
{
	...
	$.ajax({
	   ...
	   //called when complete
            ...
            if(data[0].Status=='Success')
		    {
				 ...
				 {
						if(data[0].PostOffice[j].Name == user.city)
					    {
							
							listItems2= "<option value='Other'>Other</option>";
							break Abb;
						...
						else
					    {
							...
							listItems2= "<option value='Other' selected>Other</option>";
						}
				 }
				  

				 $("#"+cityidother).val(user.city);

				 $("#"+cityid).html(listItems+listItems2);
...
function showCityOther(str1,str2)
{
		var city =  $('#'+str1).val();
...
function redeempointpage()
{
	$("#sidebar").panel("close");

	if(checkforundefined(user.firstname).trim()=='' || checkforundefined(user.firstname).trim()=='NA')
	{
		toast('Please update profile name first to proceed further.');
	}
	else if(checkforundefined(user.emailid).trim()=='' || checkforundefined(user.emailid).trim()=='NA')
	{
		toast('Please update email id first to proceed further.');
	}
	else if(checkforundefined(user.city).trim()=='' || checkforundefined(user.city).trim()=='NA')
	{
		toast('Please update city first to proceed further.');
	}
	else
	{

		$('#redeempoint').val('');
		$('#redeemwallet_1').val(user.mobile);
		
		$("#walletdiv1").css("display","block");
		$("#upidiv1").css("display","none");
		$("#accountdiv1").css("display","none");

		$('#redeemWallet1').prop('checked', true);

		$('.submitpaytmwallet_1').css("pointer-events", "");

		update_balancePts_forredeem();

	}
}

function update_balancePts_forredeem()
{
	  $.ajax({
			url: SERVER2+'get_points_json_api.asp',
			type: 'GET',
			...
			success: function(data, textStatus, xhr) {
				...
				if(data.data!='')
				{
					 ...
					 $.mobile.changePage( "#pointPage_new", { transition: "none"} );
				}
				else
				{
					 user.balance = '0';
...
function hideandshow(str)
{
	...
	if(str == 'redeemWallet1')
	{
		$("#walletdiv1").css("display","block");
		$("#upidiv1").css("display","none");
		$("#accountdiv1").css("display","none");
	}
	else if(str == 'redeemUpi1')
	{
		$("#walletdiv1").css("display","none");
		$("#upidiv1").css("display","block");
		$("#accountdiv1").css("display","none");
	}
	else if(str == 'redeemAccount1')
	{
		$("#walletdiv1").css("display","none");
		$("#upidiv1").css("display","none");
		$("#accountdiv1").css("display","block");
	}
}

function submitPaytmwallet_1(ptype)
{

	if (ptype=='wallet')
	{
		var pt_amt = $('#redeempoint').val();
		var userid = $('#redeemwallet_1').val();
		...
		else if(pt_amt=='' || pt_amt=='0' )
		{
			toast("Please enter points to redeem");
		}
		else if((parseFloat(pt_amt)) > parseFloat(user.balance))
		{
			toast('Points redeemeed cannot be greater than point balance');
		}
		else
		{
			 ...
			 $('.submitpaytmwallet_1').css("pointer-events", "none");
			 isvalid_WalletCheck_1(pt_amt,userid,'wallet');
			 //burnpoints_byqr(pt_amt,userid,'wallet');
		}
	}
	else if (ptype=='upi')
	{
toast("Coming Soon");
		// var pt_amt = $('#redeempoint').val();
		// var userid = $('#redeemupi_1').val();

		// if(userid=='')
		// {
		// 	toast("Please enter UPI ID");
		// }
		// else if(userid.indexOf("@")==0 || userid.indexOf("@")== -1 )
		// {
		// 	toast('Please enter valid UPI ID');
		// }
		// else if(pt_amt=='' || pt_amt=='0' )
		// {
		// 	toast("Please enter points to redeem");
		...
		// {
		// 	 $('.submitpaytmwallet_1').css("pointer-events", "none");
		// 	 isvalid_WalletCheck_1(pt_amt,userid,'upi');
		// }
	}
	else if (ptype=='account')
	{
		toast("Coming Soon");
		// var pt_amt = $('#redeempoint').val();
		// var userid_acc = $('#redeemaccount_1').val();
		// var ifsc_acc = $('#redeemifsc_1').val();

		// if(userid_acc.trim()=='' || ifsc_acc.trim()=='')
		// {
		// 	toast("Please enter Account number/ IFSC Code");
		// }
		// else if(pt_amt=='' || pt_amt=='0' )
		// {
		// 	toast("Please enter points to redeem");
		...
		// {
		// 	 $('.submitpaytmwallet_1').css("pointer-events", "none");
		// 	 isvalid_WalletCheck_acc_1(pt_amt,userid_acc,ifsc_acc);
		// }

	}
}

function isvalid_WalletCheck_1(pt_amt,userid,ptype)
{
	 if(ptype=='wallet')
	 {
			$.ajax({
				url: SERVER_WalletCheck,
				type: 'POST',
				timeout: 70000,
				dataType: 'json',
				headers: {
	 ...
	 else if(ptype=='upi')
	 {
			$.ajax({
				...
				success: function(data, textStatus, xhr) {
				 ...
				 {
					 toast('loading...');
					 burnpoints_byqr(pt_amt,userid,'upi');
				 }
				 else
				 {
					toast(data.msg);
...
function isvalid_WalletCheck_acc_1(pt_amt,userid_acc,ifsc_acc)
{
			...
			$.ajax({
				...
				success: function(data, textStatus, xhr) {
				 ...
				 {
					 toast('loading...');
					 burnpoints_byqr_acc(pt_amt,userid_acc,ifsc_acc);
				 }
				 else
				 {
					toast(data.msg);
...
function burnpoints_byqr(camount,userid,paytype)
{
	...
	$.ajax({
			...
			success: function(data, textStatus, xhr) {
				 ...
				 //if(data.data.length>=1)
				 if(data.data.indexOf("have redeemed")>=1)
				 {
					//toast(data.data);
					sendGratification(camount,userid,paytype,refbillno);
				 }
				 else
				 {
					 toast(data.error);
...
function burnpoints_byqr_acc(camount,userid,ifsc_acc)
{
	...
	$.ajax({
			...
			success: function(data, textStatus, xhr) {
				 ...
				 {
					//toast(data.data);
					sendGratification_acc(camount,userid,ifsc_acc,refbillno);
				 }
				 else
				 {
					 toast(data.error);
...
function sendGratification(amount,userid,ptype,refbillno)
{
    var address = user.city+' '+user.pincode;
	    ...
	    $.ajax({
				...
				success: function(data, textStatus, xhr) {
				 ...
				 console.log("data:"+JSON.stringify(data));
					...
					{
						//toast('Request accepted');
						toast_p('RS.('+amount+') will be credited to your UPI ID.');
						authenticate_new_grati();
...
function sendGratification_acc(camount,userid_acc,ifsc_acc,refbillno)
{
	var address = user.city+' '+user.pincode;
	...
	$.ajax({
				...
				success: function(data, textStatus, xhr) {
				 ...
				 console.log("data:"+JSON.stringify(data));
					...
					{
						//toast('Request accepted');
						toast_p('RS.('+camount+') will be credited to your bank account.');
						authenticate_new_grati();
...
function authenticate_new_grati()
{
	$('.submitpaytmwallet_1').css("pointer-events", "");
...
{
	//alert("asd");
	$("#Updatepopup_Message").modal({
        show: false,
        backdrop: 'static'
    }); 
	$("#Updatepopup_Message").modal("show"); 
}
